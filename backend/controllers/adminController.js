...
const addPatient = async (req, res) => {
    try {
        ...
        // Upload image to cloudinary or set default
...
const deleteDoctor = async (req, res) => {
    try {
        ...
        // Find and delete the doctor by email
...
// API for deleting a patient/user
const deletePatient = async (req, res) => {
    try {
        ...
        const deletedPatient = await userModel.findOneAndDelete({ email });
...
// API for updating doctor details by email
const updateDoctor = async (req, res) => {
    try {
        const { email } = req.params; // Doctor email passed in the request URL
        const { name, password, speciality, degree, experience, about, fees, address } = req.body;
        const imageFile = req.file;

        // Check if doctor email is provided
        if (!email) {
            return res.json({ success: false, message: "Doctor email is required" });
        }
        ...
        // Update the doctor document by email
        await doctorModel.findOneAndUpdate({ email }, updatedData, { new: true });
...
// API for updating patient details by email
const updatePatient = async (req, res) => {
    try {
        const { email } = req.params; // Patient email passed in the request URL
        const { name, password, dob, gender, contactNumber, address } = req.body;
        const imageFile = req.file;

        // Check if patient email is provided
        if (!email) {
            return res.json({ success: false, message: "Patient email is required" });
        }
        ...
        // Update the patient document by email
...
export { addDoctor, loginAdmin, allDoctors, appointmentsAdmin, appointmentCancel, adminDashboard, addPatient, allPatients, deleteDoctor, deletePatient, updateDoctor, updatePatient }